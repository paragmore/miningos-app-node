...
// Resolves consumption by featureConfig, mirroring the header UI. Central DCS
// sites take precedence and are handled in getSiteLiveStatus (site_main meter);
// otherwise: totalSystemConsumptionHeader (0) > totalTransformerConsumptionHeader > site meter
...
async function getSiteLiveStatus (ctx, req) {
  ...
  // Central DCS: a single fetch feeds both the site consumption (site_main
  // meter, same source as the energy layout view) and the miner cooling
  // system status shown in the header
